...
| 같은 팀 에이전트에 1초 안에 끝나는 일을 맡긴다 | 아니다 | 규약이 값을 할 일이 없다. 상대가 하나면 HTTP로 맞추는 편이 효과적이다 |
...
| 오래 걸리는 일 맡기고 진행 확인 | 된다 | 폴링, 스트리밍, 푸시 셋 다 있다. 폴링은 진행 중 확인이 423바이트이고 완료 응답이 571바이트라 비싸므로 스트리밍이나 푸시를 쓴다 |
...
v1.0 550이다. 위 짧은 작업 표의 581은 부하 없이 단발로 한 번 호출해 잰 값이라 조건이
다르다.
...
./harness/axis3_load_gen.sh      # 오버헤드 두 세대, 약 5시간(무인 창)
./harness/axis3_load_4.sh        # 오버헤드에 gRPC까지, 약 6시간(무인 창)
./harness/grpc_probe.py --host <IP>   # gRPC 단발 비교. grpcio 필요
```

오버헤드는 무인 창에서 돌렸다. 한 회차만 재현한다면 `axis3_load.sh` 하나면 되고 두 세대와 gRPC까지 보려면 `axis3_load_gen.sh`와 `axis3_load_4.sh`를 쓴다. 주말
창에서는 `harness/chain_axis3_gen.sh`와 `harness/chain_a2a_weekend.sh`가 이것들을
순서대로 돌렸다.

서버 소스는 `k8s/axis1/`(선언 대 실제), `k8s/axis2/`(대안 대비),
`k8s/three-arms/`(오버헤드), `k8s/four-arms/`(gRPC 포함 오버헤드),
`k8s/grpc/`(gRPC 단발)에 있다. 세 구현은 파드 하나에 컨테이너 셋이다.
