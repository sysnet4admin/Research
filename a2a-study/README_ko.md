...
A2A(Agent2Agent)는 에이전트가 다른 에이전트에 일을 넘기고 결과를 받는 방식을
...
보이는 것은 4가지다. 카드(Agent Card)는 상대가 누구이고 무엇을 하며 어디로 어떻게
...
![보이는 것은 4가지뿐](figures/surface-ko.svg)
...
사내 계획 에이전트가 외부 업체의 배송 에이전트에 경로 계산을 넘긴다면, 계획
...
한다. 흔히 쓰는 방법은 2가지이고 둘 다 대가가 있다. 먼저 상대 에이전트마다 HTTP
API를 따로 맞추는 것이다. 주소와 인증과 요청 형식을 상대별로 알아야 하고 오래
걸리는 일의 진행 확인과 취소도 상대별로 다르게 구현한다. 에이전트가 늘수록 맞출
쌍이 늘어난다. 그리고 상대 에이전트를 MCP 도구로 감싸는 방법이 있다. 도구 호출
규약은 통일되지만 도구는 "호출하면 결과가 온다"는 모델이라 진행 중인 일의
...
상대와 무관하게 한 가지로 정한다. 그래서 이 연구의 대조군은 순수 HTTP와 MCP다.
...
넘기는 방식을 표준화한다. 도구는 호출하면 결과가 오는 것이고 에이전트는 일을
...
에이전트가 MCP로 도구를 호출하는 구성을 예로 드는데, 그것은 일을 받은 쪽의 사정이지 내가
...
이 식별자로 상태 변화를 안다. 태스크 상태는 8종(SUBMITTED, WORKING, COMPLETED,
...
2026-09-10 기준 최신은 1.1.4(09-08)다. 두 세대가 나누어지는 항목은 v0.3 이름과 경로로
...
A2A를 쓰지 않았다면 실제로 썼을 두 가지와 나란히 놓고 비교한다.
...
두 가지를 조합해 조건 4개를 만든다. 구현 1개를 한 조건에서 30초 돌린 것이
...
- **일이 언제 끝나는지 알아야 할 때.** 15초짜리 작업을 넘기고 기다리는 동안 지금
  접수됐는지 처리 중인지 끝났는지 알아야 한다. A2A는 태스크 식별자 1개로 조회한다.
...
- **하던 일을 멈춰야 할 때.** 사용자가 취소를 눌렀거나 상위 작업이 실패해 하위 일을
  거둬들여야 하는 경우다. A2A는 취소가 스펙에 있고 SDK가 그것을 AgentExecutor의
...
- **상대가 중간에 되물을 때.** 어느 환경에 배포할지 같은 것을 처리 도중 물어야 하는
  ...
  호출을 처음부터 다시 하게 된다.
- **상대가 여럿이거나 바뀔 때.** 사내 에이전트 1개만 호출한다면 규약이 없어도 된다.
...
시간)이 1.2~2.1ms 더 걸린다. MCP와의 차이는 0~0.4ms로 작다. 비용의 큰 부분은 구조화된 프로토콜 공통의 것이다.

**Q. 그래서 언제 쓸 만한가?**

A. 위 네 가지 중 하나라도 해당하면 쓸모가 있다. 아래 표는 위 수치를 저자가 읽은 것이고
측정값 자체는 아니다.

| 상황 | 권하는가 | 이유 |
|---|---|---|
| 사내 서비스 하나의 도구를 호출한다 | 아니다 | MCP로 충분하다. A2A는 얻을 것 없이 응답이 7배가 되고 지연이 붙는다 |
| 같은 팀 에이전트에 1초 안에 끝나는 일을 넘긴다 | 아니다 | 규약이 값을 할 일이 없다. 상대가 하나면 HTTP로 맞추는 편이 효과적이다 |
| 오래 걸리는 일을 넘기고 진행을 보거나 취소해야 한다 | 그렇다 | 조회, 스트리밍, 푸시, 취소가 스펙에 있어 상대가 바뀌어도 클라이언트가 같다 |
...
가장 길다. 왼쪽의 얻는 것과 오른쪽 둘의 잃는 것이 맞바꾸는 관계다.

![얻는 것과 잃는 것](figures/result-ko.svg)

## 운영자가 알아야 할 것

| 하려는 것 | 되는가 | 주의 |
|---|---|---|
| 카드로 상대 찾기 | 된다 | 경로는 `/.well-known/agent-card.json` 1개다. `/.well-known/agent.json`은 404 |
| 카드에 인증 걸기 | 선언만 된다 | API 키, HTTP Bearer, OAuth2 세 종류를 다 써도 강제가 없다. 스펙은 서버가 인증하라고 하므로 앱이나 앞단이 담당해야 한다 |
...
| 오래 걸리는 일 넘기고 진행 확인 | 된다 | 폴링, 스트리밍, 푸시 셋 다 있다. 폴링은 진행 중 확인이 423바이트이고 완료 응답이 571바이트라 비싸므로 스트리밍이나 푸시를 쓴다 |
...
| 백엔드 재시작 넘겨 상태 유지 | 안 된다 | `InMemoryTaskStore`는 파드 메모리다. 재시작해도 남는 저장소로 바꿔야 한다 |
...
| 1-11 | 트랜스포트 동등성 | 어느 바인딩으로도 같은 일을 할 수 있다 | JSON-RPC와 REST와 gRPC 세 바인딩에서 같은 일을 할 수 있다. JSON-RPC와 REST는 헤더를 붙여야 v1.0이고 안 붙이면 v0.3으로 판정된다. gRPC는 처음부터 v1.0 스키마다 | △ |
...
그대로 통과된다. 스펙은 서버가 인증하라고 적었으므로 그 몫은 앱이나 앞단이 담당해야 한다.
...
이 헤더가 정한다. 헤더가 없으면 v0.3으로 판정한다. 그래서 v1.0 클라이언트가 헤더를
빠뜨리면 오류 없이 v0.3으로 취급된다. 조용히 어긋나므로 알아채기 어렵다.

나누는 방식도 바인딩마다 다르다. JSON-RPC는 주소 하나에서 헤더로만 나누어진다.
REST는 경로까지 나누어지는데 `/a2a/rest/`가 v1.0이고 `/a2a/rest/v1/`이 v0.3이라 이름과 실제가
반대로 보인다.
...
15초 작업을 넘기고 결과를 받기까지 걸린 시간은 세 구현 다 15.1~15.2초로 같았다.
나누어지는 것은 그 15초 동안 클라이언트가 무엇을 할 수 있느냐다.
...
| 취소 | 규약 없음. 이 연구 구현은 표시만 바꾼다 | 규약 없음. 이 연구 구현은 표시만 바꾼다 | 스펙에 있고 SDK가 AgentExecutor의 취소 훅에 연결한다 |
| 재시작 뒤 상태 유실을 알리는 방법 | 서버가 정한 오류 코드 | 정상 결과 문자열(코어에 해당 오류가 없어 이 연구가 그렇게 둠) | 프로토콜 오류 |

폴링만 놓고 보면 A2A가 가장 비싸다. 태스크 객체 전체를 매번 함께 보내기 때문이고
...
그러면 상태 유실과 정상 상태를 나누는 일이 클라이언트의 문자열 해석이 된다.
...
세 구현에 같은 부하를 걸어 60셀을 측정했다. 회차 `axis3-0909`. 2026-09-11부터
09-12까지 두 세대와 gRPC를 460셀로 보강했다. 회차 `axis3gen-0911A`, `-0911B`(두
세대), `axis3four-0912C`, `-0912D`, `-0912E`(gRPC 포함). 전부 오류와 버린 요청이
0이다.

A2A는 같은 일을 하는 최소 HTTP보다 호출당 p50이 1.2~2.1ms 더 걸린다. MCP보다는
0~0.4ms 더 걸린다. 60셀과 보강 460셀 전부 오류와 버린 요청이 0이었다. 조건별 수치는
아래 회차별 수치에 있다.

읽을 것은 두 수의 크기 차이다. A2A와 MCP의 간격이 작다는 것은 비용의 큰 부분이 A2A라서
붙는 것이 아니라 구조화된 프로토콜이라서 붙는다는 뜻이다. 이 측정의 MCP 대조군 기준으로 A2A는
0~0.4ms 더 걸렸다. 순수 HTTP에서 옮겨 온다면 1.2~2.1ms를 그대로 떠안는다.
...
| 조건 | HTTP | MCP | A2A v0.3 | A2A v1.0 | v0.3 - HTTP |
|---|---|---|---|---|---|
| 50rps 매번 새 연결 | 4.4 | 5.5 | 5.8 | 6.0 | +1.4 |
| 50rps 연결 재사용 | 3.4 | 5.5 | 5.5 | 5.7 | +2.1 |
| 100rps 매번 새 연결 | 3.6 | 4.4 | 4.8 | 4.8 | +1.2 |
| 100rps 연결 재사용 | 3.0 | 4.2 | 4.5 | 4.5 | +1.5 |

성공 응답 1건당 바이트는 100rps 연결 재사용에서 HTTP 81, MCP 287, A2A v0.3 584,
v1.0 550이다. 위 짧은 작업 표의 581은 부하 없이 단발로 한 번 호출해 측정한 값이라 조건이
다르다.

**두 세대의 지연 차이는 0~0.2ms다.** 조건 넷 중 둘은 같은 값이다. 세대를 바꿔도
지연은 움직이지 않고 바이트만 v1.0이 34바이트 작다. 상태 이름이 길어졌지만 파트와
메시지의 `kind` 필드가 빠져서 총량이 줄었다.

A2A와 MCP의 p50 차이는 0~0.4ms로 작다. 비용의 큰 부분은 구조화된 프로토콜 표면
공통의 것이다.

gRPC 바인딩은 별도 회차에서 측정했다. 같은 A2A 컨테이너가 JSON-RPC와 gRPC를 함께 열도록
구성을 바꾼 회차라 위 표와 자원 조건이 다르므로 섞어 읽지 않는다. 그 회차 안에서
100rps 연결 재사용 기준으로 gRPC가 p50 3.1ms에 직렬화 280바이트이고 같은 조건의
JSON-RPC v1.0이 4.9ms에 550바이트다. 다만 생성기의 gRPC 경로는 채널을 계속 재사용해
연결 방식 축이 걸리지 않았고 바이트도 protobuf 직렬화 길이라 HTTP 본문 바이트와 같은
잣대가 아니다. 상세는 `RESULTS.md`의 축 3 보강 절에 있다.

## 한계

- SDK 하나(Python 1.1.2)와 샘플 AgentExecutor 기반 서버로 측정했다. 다른 언어
  SDK와 실제 서비스용 AgentExecutor는 다를 수 있다.
...
- gRPC 회차는 생성기가 채널을 계속 재사용해 연결 방식 축이 걸리지 않았다. 연결
  재사용 행에서만 읽는다. 바이트도 protobuf 직렬화 길이라 HTTP 본문 바이트와 같은
  잣대가 아니다.
- gRPC를 포함한 회차는 A2A 컨테이너가 JSON-RPC와 gRPC를 함께 열어 앞 회차보다
  전반적으로 0.4~0.5ms 느리다. 두 세트를 섞어 비교하지 않는다.
- 대안 대비의 A2A 호출은 `A2A-Version` 헤더 없이 보내 v0.3 형식 값이다. 2026-09-11
  보강에서 같은 항목을 v1.0 형식으로 다시 측정했고 상세는 `RESULTS.md`의 축 3 보강
  절에 있다. 오버헤드는 두 세대를 다 측정했다.
...
./harness/axis3_load.sh          # 오버헤드, 약 3.5시간. 결과 runs/axis3-<날짜>/
./harness/axis2_v10_bytes.sh     # 대안 대비의 A2A를 v1.0으로, 약 3분
./harness/axis3_load_gen.sh      # 오버헤드 두 세대, 약 5시간
./harness/axis3_load_4.sh        # 오버헤드에 gRPC까지, 약 6시간
./harness/grpc_probe.py --host <IP>   # gRPC 단발 비교. grpcio 필요
```

오버헤드는 호스트에 다른 작업 없이 돌렸다. 한 회차만 재현한다면 `axis3_load.sh` 하나면 되고 두 세대와 gRPC까지 보려면 `axis3_load_gen.sh`와 `axis3_load_4.sh`를 쓴다. `harness/chain_axis3_gen.sh`와 `harness/chain_a2a_weekend.sh`가 이것들을 순서대로
돌린다.

서버 소스는 `k8s/axis1/`(선언 대 실제), `k8s/axis2/`(대안 대비),
`k8s/three-arms/`(오버헤드), `k8s/four-arms/`(gRPC 포함 오버헤드),
`k8s/grpc/`(gRPC 단발)에 있다. 세 구현은 파드 1개에 컨테이너 3개다.
