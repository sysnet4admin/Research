# What agentgateway actually enforces and observes for A2A
...
[agentgateway's MCP surface](../agentgateway-study/): a declared capability is
...
by `../agentgateway-study/test-cluster/`) with the gateway install scripts in
`../agentgateway-study/harness/`. The v1.4.1 round used the
...
- The backend is a purpose-built minimal agent, not an official A2A SDK
  server. It implements the surface the gateway inspects (card GET, JSON-RPC
  POST); SDK-specific card composition may differ.
