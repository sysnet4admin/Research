"""A2A 구현에 gRPC 바인딩을 더한 것. axis2의 srv_a2a.py 최소 델타 사본이다.

[gRPC 델타] 같은 핸들러에 gRPC 서버를 포트 9111로 하나 더 띄우고 카드에 GRPC
인터페이스를 선언한다. 1단계에서 gRPC를 재지 않아 트랜스포트 동등성 판정에
"gRPC는 재지 않았다"가 남아 있었고 그것을 없애려는 것이다.
