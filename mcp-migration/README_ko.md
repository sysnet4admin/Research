...
레플리카가 늘고 파드가 교체되는 환경에서 실제로 얼마나 차이가 나는지 측정한
자료다.

그래서 직접 측정했다. 같은 워크로드를 두 번 돌렸다. 한 번은 구 스펙(2025-11-25)
...
- 실행 중에 파드 1개를 강제로 종료하는 측정에서 구 스펙은 6회에 요청
...
| 지금 포팅하는 중이다 | 성패는 핸들 설계에서 나누어진다. 상태를 파드 메모리에 두면 안 된다. 핸들 값에 서명해 담거나(HMAC) 외부 저장(Redis)에 둔다. 둘 다 스케일아웃과 파드 교체 양쪽에서 유실 0이었고 파드 메모리는 양쪽 다 잃었다 |
...
- **B (신 스펙, 2026-07-28)**: 같은 워크로드 부분집합(echo, get-sum, 카운터)을
  ...
  구현에 넘기기 때문에 B는 세 가지로 구현했다. 파드 메모리(단순 포팅),
  상태를 핸들 값에 서명해 담는 HMAC 방식, Redis 외부 저장이다.

측정은 M1, M2, M3 세 가지이고 여기에 게이트웨이 경로를 따로 측정했다. 조건
조합 1개를 셀이라고 부른다. 클라이언트에서 서버까지
가는 길은 (a) LoadBalancer Service 직접, (b) Gateway API 구현체 경유,
(c) agentgateway 경유 세 가지를 두고 이 중 (a)와 (c)를 측정했다.
...
- **M3 파드 교체**: 60초 실행의 20초 시점에 파드 1개를 강제로 종료한다.
...
가지고 있고 연결이 어느 파드로 가는지에 따라 결과가 달라지기 때문이다. 그에
...
- **게이트웨이를 거치면 같은 실패가 다른 상태 코드로 나타난다.** 직접 경로에서 세션을
  ...
  뒤 5xx에서도 재초기화하도록 고쳐서 다시 측정했다.
- **게이트웨이 셀의 절대 처리량을 직접 경로와 나란히 비교하면 안 된다.**
  게이트웨이 컨트롤 플레인과 프록시가 워커 CPU를 쓰기 때문에 스케일아웃
  셀은 한 번에 한 서버만 레플리카 4로 올려서 측정했다. 파드 교체 비교만 같은
  조건(레플리카 2, 100rps)으로 게이트웨이 경유와 직접 경로를 잇달아 측정했다.
...
60초 실행 중에 파드 1개를 강제로 종료하는 측정을 구 스펙에 6회, 신 스펙에
3회 했다. 구 스펙은 요청 1,922건을 세션 유실로 잃었고 회차별로는 59건에서
1,072건까지 갈렸다. 종료된 파드가 세션을 몇 개나 가지고 있었는지에 따라 달라지기
때문이다. 그에 반해 신 스펙은 3회 모두 요청 하나 잃지 않았다. 롤아웃과 노드 드레인,
오토스케일링의 축소에서는 파드 종료가 상시로 일어나므로 이 차이가 운영 환경에서도
그대로 나타날 조건이 갖춰져 있다.

핸들 설계에도 같은 시험을 했다. 파드 메모리 구현은 파드가 종료되면 그 파드가
가지고 있던 상태가 모두 사라지므로 5회에 핸들 유실이 13,942건 나왔다. HMAC
...
### 4. 이관의 성패는 핸들 설계에서 나누어진다

스펙은 핸들 지속성을 규정하지 않고 구현에 넘긴다. 서버를 포팅하고도 핸들
...
먼저 쿠버네티스 서비스의 스티키 세션이 있다. `sessionAffinity: ClientIP`를
켜자 레플리카 4에서 200.0rps에 유실 0이 됐고(끄면 중앙 26.4rps에 유실
13,760) 파드 교체에서도 유실이 2,148건에서 88건으로 줄었다. 다만 이
측정에는 전제가 하나 붙는다. 부하 생성기가 호스트 1대라 클라이언트 IP가
하나였고 ClientIP 친화도는 IP 단위로 고정하므로 트래픽 전부가 파드 1개로
몰렸다. 세션이 전부 살아남은 것은 부하 분산을 전부 포기한 결과다. 실제
환경은 클라이언트 IP가 여러 개라 효과가 부분적일 것으로 보인다.

그리고 게이트웨이가 세션을 대신 담당하는 방식이 있다. 클라이언트의 세션은
...
초기화하는 클라이언트로 측정한 것이다. 게이트웨이 뒤에서는 세션을 잃은 요청이
...
지워지기 때문이다. 그리고 이미지 빌드에는 Docker 데몬이 실행 중이어야
한다. 데몬을 손으로 켜는 환경(colima 등)이라면 빌드 스크립트가 먼저 확인해서
없으면 켜 보고 그래도 안 되면 중단하도록 해 두었다. 이 전제를 놓쳐서 한 회차를 그대로 날린 뒤에 넣은 확인이다. 마지막으로 집계 표는 이 README에 있고
