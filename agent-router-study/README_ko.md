...
agentgateway는 MCP 인가 정책(`mcpAuthorization`)에서 호출 인자를 볼 수 없고 그것을 이
저장소가 실측해 업스트림에 제보했다(`#3092`). 그러니 이쪽은 어떻게 푸는지가 첫 질문이다.
...
agentgateway는 MCP 인가 정책에서 이것을 하지 못하고 라우트 단위 정책에 따로 적어야 한다.
그 경로는 PR #3301(2026-09-03 머지)로 열렸고 이 저장소가 측정한 v1.5.0에는 없다.
