...
> **Extended 폭(13)의 구성(프로파일 경계)**: HTTP 프로파일 Extended(standard 채널) +
> 별도 프로파일의 standard 기능 일부(BackendTLSPolicy, GRPCRoute)를 "구현체가 추가로
> 제공하는 optional 역량"으로 함께 편입한다. backend-tls와 grpc-routing은 각자 프로파일
> (backendTLS, GRPC) 기준으론 Core지만, HTTP 적합성 관점에선 선택 역량이라 breadth에 넣는다.
> 셋(BackendTLS/GRPCRoute/HTTPListenerIsolation 포함) 모두 채널은 standard로 v1.4.0 소스 확인됨.
