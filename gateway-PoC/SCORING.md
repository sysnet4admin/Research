# Gateway PoC 채점 설계 (동결 기준)

이 문서는 재측정 채점의 **단일 진실원**이다. 가중치, 레벨 분류, 임계값, 등급
정의는 **재측정 실행 전에 확정하고 동결**한다. 결과를 보고 기준을 조정하지
않는다(측정 무결성 원칙).
...
## 3. 17개 테스트 레벨 분류 (v1.4.0 소스 확정, 동결)
...
1. grpc-routing 폴백이 백엔드 "존재"만으로 PASS 반환(약 1180행). 실제 gRPC
   호출 성공을 요구하도록 수정.
2. README 공식과 스크립트 공식 불일치 해소(단일 공식으로 동결).
...
근거: 7개 구현체의 공식 conformance 리포트를 교차검증한 결과, **강한 코호트가
공통으로 conformance 근거를 갖는 최고 버전이 v1.4**다. Istio와 Cilium이
v1.5 리포트가 아직 없어 천장을 v1.4로 고정한다. v1.5로 올리면 이 둘은 근거 없이
측정하는 셈이라 비교 공정성이 깨진다. 이 판정은 test-cluster의 Cilium 1.19.4
(=v1.4 도입 라인)와도 일치한다. 작년 v1.2.0(Cilium 1.18.4 천장)에서 한 단계 상향.

### 9.2 구현체 지원 매트릭스 (2026-06)

| 구현체 | 최신 버전 | 최고 CORE conformance | 채널 |
|---|---|---|---|
| NGINX Gateway Fabric | v2.6.3 | v1.5 | standard |
| Envoy Gateway | v1.8.0 | v1.5.1 | standard |
| Istio | 1.30.0 | v1.4.0 | experimental |
| Cilium | 1.19.4 | v1.4.0 | experimental(확인要) |
| Traefik | v3.7.1 | v1.5.1 | experimental |
| kgateway | 2.3.x | v1.5.1 | experimental |
| Kong | KGO 2.0.6 | v1.3.0(CORE 부분통과) | standard |
...
Kong은 v1.4/v1.5 conformance 리포트가 없고, 최선(KGO 2.0.6)이 v1.3.0에서
**CORE 부분통과**(HTTPRouteWeight 실패 = 우리 canary-traffic Core 항목에 대응).
substrate를 낮추지 않고 다른 6종과 동일한 v1.4에서 테스트하며, 부분통과/실패를
**결과로** 기록한다. 작년 F와 일관. 단 구버전 KIC(unmanaged) 대신 신규
**KGO managed gateway** 경로로 설치해 측정한다.
...
> 미확인 1건: Cilium 1.19.4의 v1.4 conformance 리포트 채널(standard/experimental)은
> 구현 직전 공식 리포트로 최종 확인한다. target=v1.4 판정 자체는 채널과 무관하게
> 유효(Cilium은 어느 채널이든 v1.4가 천장).
