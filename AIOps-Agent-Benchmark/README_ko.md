...
> **오픈소스 모델로 평가 확장 진행 중.** 이 벤치마크를 단일 워크스테이션 범위(80~96GB GPU 1장 또는 128GB 워크스테이션)에서 동작하는 자체 호스팅 오픈소스 모델로 확장하고 있습니다. Gemma, Llama, Qwen 등의 오픈소스 계열을 대상으로 삼되 여기에 한정하지 않습니다. 같은 시나리오와 Ops_Score를 그대로 적용하므로, 아래 상용 기준선과 견줄 수 있습니다. 현재는 초기 프로토타입 단계이며 결과는 아직 공개하지 않았습니다.
...
같은 열 개 시나리오와 같은 Ops_Score를, 단일 워크스테이션 범위에서 로컬로 구동하는 Gemma 4 계열 오픈 모델에 그대로 적용해 초기 프로토타이핑을 진행했습니다. 모델별 수치를 말하기엔 이르지만, 격차의 윤곽은 이미 드러납니다.
...
모델별·티어별 전체 결과는 아직 남아 있습니다. 시나리오와 채점이 공개돼 있어, 작업이 이어지는 동안에도 비교는 재현 가능합니다.
