#!/usr/bin/env bash
# rv 체인7 (2026-09-04 금): 18:00 대기 -> 게이트 -> 혼합 백엔드(rv_mixed.sh) -> 0ms 지속 창(rv_tail.sh 스윕 생략) -> guardrail 지속 창(rv_grsus.sh).
# chain_rv4.sh의 최소 델타 사본(단계 1개). 상주 환경(v1.5.0 게이트웨이, mcp-b) 재사용.
# 사용: caffeinate -i nohup ./chain_rv7.sh > /tmp/rv-chain7.log 2>&1 &
set -uo pipefail
...
CKPT="$STUDY/runs/.rv7-ckpt"
log() { echo "[chain7 $(date '+%m-%d %H:%M')] $*"; }
push() { ( cd "$REPO" && git add agentgateway-study/runs >/dev/null 2>&1 \
  && git commit -q -m "rv 체인7: $1" >/dev/null 2>&1 \
...
run_stage mixed "혼합 백엔드"        ./harness/rv_mixed.sh "$PY" runs/rv-mixed-0904
run_stage tail0 "0ms 지속 창"        env RV_SKIP_SWEEP=1 RV_SUSTAIN_DELAY=0 RV_SUSTAIN_REPS=2 ./harness/rv_tail.sh "$PY" runs/rv-tail0-0904
run_stage grsus "guardrail 지속 창"  ./harness/rv_grsus.sh "$PY" runs/rv-grsus-0904
log "=== 체인7 전체 종료 ==="
