#!/usr/bin/env bash
# [rv 사본, reuse 모드] rv_run_ab.sh의 최소 델타 사본: conn-mode reuse, 파일 접두사 abr-. 원측정 = weekend-0820 3부.
# A/B 추가 측정: 게이트웨이 유무 비교 (2026-08-19, ONE Summit CFP 보강).
#
# arm-direct: loadgen -> mcp-b Service(LB) 직접 (주소는 svc에서 동적 조회)
# arm-gw:     loadgen -> agentgateway v1.5.0 -> mcp-b (정책 0개)
#
...
# 고르게 분산한다.
#
# 사용: ./rv_run_abr.sh <PYTHON> <OUT_DIR>
set -uo pipefail
...
COOLDOWN="${RV_COOLDOWN:-180}"
mkdir -p "$OUT"

log() { echo "[$(date '+%m-%d %H:%M')] $*"; }
note() { echo "$*" >> "$OUT/FINDINGS.md"; }

echo "# A/B 측정 reuse 모드: 게이트웨이 유무 (자동 생성)" > "$OUT/FINDINGS.md"
note ""
...
note "reuse(연결 재사용) 모드, 셀 간 쿨다운 ${COOLDOWN}초, 회차 안에서 direct -> gw 교대 실행."
note ""

# ── 준비: 게이트웨이 설치 (이후 두 arm 내내 유지) ─────────────────────
log "agentgateway v1.5.0 설치 (두 arm 내내 유지)"
AGW_VER=v1.5.0 bash "$DIR/install_agw.sh" > "$OUT/install.log" 2>&1
sleep 30
...
cell() { # cell <name> <url> <rps> <concurrency>
  "$PY" "$LOADGEN" --url "$2" --dialect b --tool echo \
    --concurrency "$4" --duration "${RV_DURATION:-30}" --conn-mode reuse --rps "$3" \
...
for rps in 100 200; do
  ...
  note ""
  for n in $(seq 1 "${RV_ROUNDS:-5}"); do
    log "=== ${rps}rps rep$n: direct ==="
    R=$(cell "abr-direct-rps${rps}-n${n}" "$DIRECT_URL" "$rps" "$conc")
    note "- direct n$n: $R"
    sleep $COOLDOWN
    log "=== ${rps}rps rep$n: gw ==="
    R=$(cell "abr-gw-rps${rps}-n${n}" "$GW_URL" "$rps" "$conc")
    note "- gw     n$n: $R"
    sleep $COOLDOWN
  done
  note ""
...
echo "[rv] 게이트웨이 유지 (체인 다음 단계가 이어받음)" >> "$OUT/install.log"
note "---"
