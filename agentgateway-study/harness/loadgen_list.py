# [사본] mcp-migration harness/loadgen.py에 tools/list 워크로드(--tool list)만 추가.
# 목록 필터링 비용(도구 수 x 정책 유무) 측정용. 나머지는 원본과 동일.
#!/usr/bin/env python3
...
class WorkerB:
    ...
    async def call_once(self):
        if self.tool == "list":
            ...
            try:
                r = await self.client.post(
                    self.url, headers=self._headers("tools/list"),
            ...
            if obj is None:
                ...
                self.stats.ok += 1
                self.stats.latencies.append(dt)
                self.stats.tools_count = len(obj.get("result", {}).get("tools", []))
...
async def run(args):
    ...
    out = {
        ...
        "tools_count": getattr(stats, "tools_count", None),
        "body_bytes": getattr(stats, "body_bytes", None),
    }
    return out
...
def main():
    ...
    p.add_argument("--tool", default="echo", choices=["echo", "get-sum", "counter_mem", "counter_hmac", "counter_redis", "list"])
