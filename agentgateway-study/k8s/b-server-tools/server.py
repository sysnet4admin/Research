# [목록 비용 사본] b-server-delay/server.py(echo 지연 파라미터)에 더미 도구 N개 등록만 추가.
# B_EXTRA_TOOLS 환경변수(기본 0)만큼 tool_0000.. 을 등록한다. 목록 필터링 비용 측정용.
# B_DELAY_MS 환경변수(기본 0)만큼 echo가 자기 시간을 쓴다. 나머지는 원본과 동일.
...
_DELAY_S = float(os.environ.get("B_DELAY_MS", "0")) / 1000.0
_EXTRA_TOOLS = int(os.environ.get("B_EXTRA_TOOLS", "0"))


def _register_extra(n: int) -> None:
    for i in range(n):
        def _mk(i):
            def f(message: str) -> str:
                return f"tool_{i:04d}: {message}"
            f.__name__ = f"tool_{i:04d}"
            f.__doc__ = f"Dummy tool {i} for list-size measurement"
            return f
        mcp.tool(name=f"tool_{i:04d}")(_mk(i))


_register_extra(_EXTRA_TOOLS)
