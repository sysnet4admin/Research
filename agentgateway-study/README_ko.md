...
관측, 라우팅, 보안 통제가 없으니 게이트웨이 1개가 서버를 고치지 않고 그
프로토콜 전부에 그것을 붙여 준다는 것이다. 1.0.0은 2026-03-16에 나왔고 이
연구가 측정한 버전은 v1.4.1(2026-07-29)과 v1.5.0(2026-08-27)이다.

실행 방식은 2가지다. 단독 바이너리로 띄우면 파일로 설정한다. 이 연구가 잰
...
프로토콜을 이해한다. JSON-RPC를 파싱하고 여러 서버를 엔드포인트 1개로
...
1. **도구 이름 화이트리스트는 호출 차단과 목록 필터링을 다 한다.** echo만
   ...
   않는다. 서버에 도구가 8, 100, 500개일 때 도구 1개만 남기는 정책을 거친
...
5. **traceparent는 게이트웨이를 넘어 이어지고 `_meta`에도 실린다.**
   ...
   문서에서는 이 동작을 찾지 못했다. 트레이싱을 켠 조건도 v1.5.0에서 확인했다.
   OTel 수집기로 스팬을 100% 내보내는 상태에서 MCP 경로와 일반 HTTP 경로에
   3트레이스씩 보냈더니 여섯 건 전부 게이트웨이가 내보내는 스팬이 받은 스팬
   아래로 중첩됐다. 업스트림 #2904가 보고한 부모 관계 문제는 두 경로 모두에서
   재현되지 않는다(#3059 수정 반영 뒤의 동작). 스팬 내보내기의 지연 비용은
   100rps에서 p50 차이 0.2ms 안이라 측정 잡음과 구분되지 않는다.
...
8. **인자 단위 통제는 extAuth와 extProc로도 된다.** 같은 규칙("get-sum은
   a==1일 때만")을 외부 인가 서버(extAuth, HTTP 모드)와 Envoy ext_proc gRPC
   서버로 각각 강제했다. 둘 다 a=2만 403으로 막고 a=1과 무관 도구와
   `tools/list`는 통과시킨다. 거부 형태는 나누어진다. extAuth는 검사 서버가
   돌려준 본문이 그대로 클라이언트에 나가고 extProc는 `denied by ext_proc:`
   접두사가 붙는다. 검사 서버를 내렸을 때도 다르다. extAuth는 FailClosed에서
   403, FailOpen에서 통과이고 extProc는 FailClosed에서 500이다. 비용은
   호출당 p50 +0.4~0.5ms로 mcpGuardrails와 같은 급이다(아래 표). 즉 인자
   통제 경로가 셋이고 정책(인가) 경로만 안 된다.
9. **A2A 표면에서 게이트웨이는 재작성하고 관측하며 홉 비용이 붙지만 강제는
...
| 도구 인자 기반 통제 ("delete는 막되 prod만") | 인가 정책으로는 안 되고 검사 서버 세 경로로는 된다 | 정책은 수용된 채 백엔드가 잠기고 `has(...)` 가드는 조건을 잃는다. mcpGuardrails, extAuth, extProc 모두 서버를 직접 구현해야 하고 호출당 p50 1ms 아래가 더 든다. 거부 형태와 장애 시 동작은 셋이 다르다(아래 표) |
...
| 분산 트레이싱 연계 | 된다 | 헤더와 `_meta` 양쪽으로 전파된다. 게이트웨이 트레이싱을 켠 조건에서도 스팬 부모 관계가 정상이고 100% 샘플링의 지연 비용은 잡음 안이다 |
...
guardrail 셀은 9/2~9/3 한 창에서, reuse A/B는 9/3의 두 번째 창에서 측정했다. 연결 방식과 쿨다운은 표마다
...
100rps 게이트웨이 셀 1개는 p99가 139ms였다(요청 11건 shed). 중앙값에는
영향이 없고 원인은 조사하지 않았다.
...
포화 상태의 고처리량 조건을 측정한 것이다. 그 행의 짝 증분이 다섯 행 중
...
측정 설계 기록 하나(v1.4.1 회차). 교대 이전의 순차 측정(off 경로 전체를 측정한 뒤 on 경로)은
"매번 새로 +1.0~1.1ms, 재사용 0과 구분 불가"라는 다른 그림을 냈는데, 교대
측정에서 재현되지 않았다. 경로 사이의 시간 드리프트가 한쪽 증분을 부풀리고
다른 쪽을 가린 것으로 판정하며 위 교대 수치를 정본으로 삼는다. off/on
비교처럼 차이가 1ms 아래인 측정은 경로 순차가 아니라 쌍 교대로 설계해야
한다는 것이 이 기록의 교훈이다.

![무엇을 더하면 지연이 얼마나 느는가: 통제 쌍별 전후 비교](figures/path-cost-ko.svg)

도구 수와 목록 필터링(2026-09-05). 서버의 도구가 8, 100, 500개일 때(응답
3.0KB, 36KB, 181KB) `tools/list`를 세 경로에서 측정했다. 직접, 게이트웨이(정책
없음), 게이트웨이(도구 1개만 남기는 화이트리스트). 20rps에 동시 8(100rps에서는
...
90셀 전부 20rps 달성에 오류 0. 읽기는 3가지다. 필터링은 게이트웨이에서
...
버렸고(빠른 쪽 21.8%, 느린 쪽 15.3%) 느린 쪽의 p99가 3.2초에 닿았다.
게이트웨이는 오류 0이었고 완료된 요청은 빠른 그대로였다(빠른 쪽 p95
...
똑같은 양을 버렸고(빠른 쪽 21.8%, 느린 쪽 15.2%, 느린 쪽 p99 3.2초)
게이트웨이 경로 재측정도 같았다(완료 요청 140,577건 중 HTTP 500 한 건). 한계는
게이트웨이가 아니라 이 실험실의 지속 연결 수립률(호스트 생성기 또는
노드)이고 두 경로의 차이는 빠른 쪽 p50 +0.7ms, 곧 평소의 홉이다. 이 부하
범위에서 지속 혼합 부하에 먼저 무너진 것은 게이트웨이가 아니라
실험실이었다.

### 트레이싱을 켠 경로 (2026-09-09)

OTel 수집기(0.160.0)를 같은 클러스터에 두고 `AgentgatewayPolicy`의 tracing으로
OTLP gRPC 4317에 100% 샘플링으로 보냈다. 100rps, 30초 셀, 켬과 끔 5회 교대,
오류 0.

| 모드 | 끔 p50 | 켬 p50 | 켬 - 끔 | 끔 p99 | 켬 p99 |
|---|---|---|---|---|---|
| 매번 새 연결 | 7.5 | 7.5 | +0.0 | 14.2 | 14.2 |
| 연결 재사용 | 4.7 | 4.5 | -0.2 | 13.0 | 12.5 |

값은 셀 5개의 중앙값(ms)이고 회차별 차이는 -0.4에서 +0.1ms로 부호가 나누어진다.
수집기가 같은 클러스터에 있으므로 원격 수집기에서는 다를 수 있다.
...
단위는 ms이고 정책을 켠 셀과 끈 셀의 p50 중앙값 차이다. extAuth는 검사 서버의
응답 본문이 클라이언트에 그대로 나가므로 사유 문자열에 내부 정보를 담지 않는다.
...
- **재측정(2026-09-02)**: 새 클러스터의 agentgateway v1.5.0과 Kubernetes
  v1.37.0에서 전체 매트릭스를 사전 점검 게이트를 거쳐 다시
...
- **도구 수와 목록 필터링(2026-09-05)**: 도구 8, 100, 500개에서
  `tools/list`를 세 경로(직접, 게이트웨이, 게이트웨이 + 화이트리스트)로 90셀,
  20rps.
...
- **트레이싱을 켠 경로(2026-09-09)**: 같은 클러스터의 OTel 수집기로 스팬을
  100% 내보내며 부모 관계를 두 경로에서 확인하고(6트레이스) 켬과 끔을 5회
  교대해 비용을 측정했다(20셀).
- **extAuth와 extProc(2026-09-09)**: 자작 검사 서버 둘로 같은 인자 규칙을
  강제하고 거부 형태와 검사 서버 정지 시 동작을 확인한 뒤 켬과 끔 교대로
  비용을 측정했다(40셀).
...
- 백엔드 구현 1종(에코 서버를 0ms와 200ms 두 변형으로 돌림)이고
  ...
  100rps에서 포화시킨다).
- 인자 단위 통제의 세 경로(mcpGuardrails, extAuth, extProc)는 모두 자작 최소
  검사 서버로 측정했다. 실제 정책 엔진을 붙이면 비용이 다를 수 있다.
- MCP 표면 안에서도 인증(MCP Auth)은 범위 밖이다.
- 절대 지연 수치는 VirtualBox 환경 값이다.
- 트레이싱은 수집기를 같은 클러스터에 두고 100% 샘플링으로만 측정했다. 원격
  수집기, 낮은 샘플링, 높은 부하 조건은 시험하지 않았다.
...
사전 점검 게이트가 붙은 실행 오케스트레이터, `harness/rv_read.py`가 표를
출력한다. guardrail 정책 서버는 `k8s/guardrail/`에 있다. 측정
원자료(`runs/`)는 이 저장소에 포함하지 않는다. 발행 수치는 위 표가 전부
함께 정리해 두었고 직접 측정하면 `harness/chart.py`가 원자료에서 그림을
...
`harness/uninstall_agw.sh`가 담당한다. v1.4.1 회차는
