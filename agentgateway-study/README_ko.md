...
| 도구 인자 기반 통제 ("delete는 막되 prod만") | 정책으로는 안 되고 guardrail로는 된다 | 정책은 수용된 채 백엔드가 잠기고 `has(...)` 가드는 조건을 잃는다. mcpGuardrails는 gRPC 서버를 직접 구현해야 하며 거부가 200 + JSON-RPC 오류로 나간다 |
...
- **인자 통제 대안 경로(축 3)**: ExtMcp gRPC 최소 정책 서버(`k8s/guardrail/`)로
  인자 조건 강제, 거부 형태, FailClosed 동작을 확인.
