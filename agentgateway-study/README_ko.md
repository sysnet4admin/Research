...
2. **도구 인자 조건을 쓴 정책은 수용되지만 백엔드 전체를 잠근다.** 이 연구의
   ...
   조용히 퇴화한다(막으려던 a=2도 통과, 실측). 즉 이 정책 경로 안에서는 인자
   통제의 워크어라운드가 없다. 운영자는 인자 통제가 걸렸다고 믿게 되는데
   실제로는 전면 중단이거나 조건 소실이다.
...
| 도구 인자 기반 통제 ("delete는 막되 prod만") | 안 된다 | 설정은 수용되지만 백엔드 전체가 잠긴다. `has(...)` 가드로 우회하면 조건이 사라진다(이름 전용으로 퇴화). 문서상 후보는 외부 프로세서(mcpGuardrails)이나 이 연구에서는 검증하지 않았다 |
