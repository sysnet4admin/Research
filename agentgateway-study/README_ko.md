...
3.0KB, 36KB, 181KB) `tools/list`를 세 팔에서 쟀다. 직접, 게이트웨이(정책
...
100rps를 거는 동안 빠른 백엔드를 100rps로 쟀다. 직접 경로와 게이트웨이
...
- **혼합 백엔드와 추가 지속 창(2026-09-04~05)**: 같은 게이트웨이 뒤에
  ...
  백엔드를 쟀다(40셀 + 30분 4셀). 이어서 0ms의 30분 창(8셀)과 guardrail
...
- 속도가 다른 백엔드 혼합은 부하 한 점(각 100rps)에서만 쟀다. 30분 close
