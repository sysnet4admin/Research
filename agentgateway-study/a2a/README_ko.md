...
## A2A는 무엇이고 게이트웨이는 그것으로 무엇을 하며 왜 재는가

A2A(Agent2Agent)는 에이전트가 다른 에이전트를 속을 모르는 상대로 부르는
프로토콜이다. 불리는 쪽이 에이전트 카드(`/.well-known/agent.json`)로
엔드포인트, 스킬, 인터페이스를 광고하고 부르는 쪽이 그 엔드포인트로
JSON-RPC 메시지를 보낸다. 오래 걸리는 작업은 태스크와 스트리밍으로
다룬다. 구글이 2025년에 공개했고 스펙 v1.0이 2026-03-12에 나오면서
카드의 엔드포인트 필드가 최상위 `url`에서 `supportedInterfaces[]`로
바뀌었으며 2026-08-19에 Agentic AI Foundation의 호스티드 프로젝트가 됐다. agentgateway의
A2A 지원은 Kubernetes Service의 스위치 하나(`appProtocol:
agentgateway.dev/a2a`)다. 켜면 게이트웨이가 카드의 엔드포인트를 자기
주소로 재작성하고 JSON-RPC 교환을 파싱해 액세스 로그에 남긴다. 그 밖의
A2A 전용 정책은 없다(아래 실측). 도입을 검토하는 쪽이 가장 먼저 알아야 할
사실이 이것이다.

이 연구는 MCP 연구와 같은 방식으로 그 표면을 잰다. 게이트웨이가 무엇을
재작성하고 무엇을 강제하며(결과는 없음), 무엇을 관측하고 홉이 얼마인지를
위에 적은 버전에서 확인한다.

## 도입하면 무엇이 좋아지고 어떤 비용이 드는가

- **성능 이득은 없다.** A2A 에이전트 앞에 게이트웨이를 두어도 호출이
  빨라지거나 꼬리가 안정되지 않는다. 이 초경량 에코 에이전트에서는 모든
  조건에서 게이트웨이 경유 p99가 직접 경로보다 3~6ms 높다.
- **비용은 홉 하나에 약간의 프로토콜 처리다.** 홉은 호출마다 새 연결이면
  p50 +3.0ms, 연결을 재사용하면 +0.8~1.1ms. A2A 처리(appProtocol)를 켜면
  재사용 조건에서 +0.1~0.2ms가 더 들고 새 연결 조건에서는 측정되지 않는다.
  실제 에이전트 작업(LLM 호출, 도구 실행) 앞에서는 전부 사라지는 크기다.
- **얻는 것은 디스커버리 통제와 관측이고 인가는 아니다.** appProtocol을
  켜면 에이전트 카드가 게이트웨이 주소로 재작성되어 카드로 에이전트를
  찾는 클라이언트가 라우팅 계층 위에 머물고 액세스 로그에 상태 코드
  모니터링이 놓치는 A2A 필드(메서드, 결과, JSON-RPC 오류 코드)가 남는다.
  v1.5.0의 A2A에는 요청 인가가 없다. 데이터 플레인 `A2aPolicy`는 빈
  구조체다. 관측은 있고 강제는 없다.
- **믿기 전에 확인할 것 둘.** 카드 재작성은 옵트인이고 v0.3 `url`과 v1.0
  `supportedInterfaces`를 병기한 전환기 카드는 뒤쪽만 재작성된다. v0.3
  독자는 백엔드의 직접 주소를 받아 게이트웨이를 그대로 우회한다(우회 호출
  5/5 성공). 게이트웨이 경유로 카드를 조회해 실제로 광고되는 주소를 읽어야
  한다.
...
agentgateway v1.4.1(Kubernetes v1.36.2, 2026-08-27)에서 처음 측정했고
v1.5.0(Kubernetes v1.37.0, 새 클러스터, 2026-09-02~03)에서 다시 측정했다.
결정론 프로브는 전부 같은 결과였다.

| 프로브 | v1.4.1 | v1.5.0 |
|---|---|---|
| 카드 매트릭스(appProtocol x v0.3 / v1.0 / 병기) | 병기 카드의 직접 `url` 유지 | 동일 |
| 광고된 직접 주소로 우회 | 5/5 에이전트 도달 | 동일 |
| A2A 요청 인가 | 없음(`A2aPolicy` 빈 구조체) | 동일 |
| traceparent | trace-id 유지, 게이트웨이 span-id, 본문 무주입(5/5) | 동일 |
| 오류 형태와 액세스 로그 | HTTP 200 + -32601, `a2a.response.error_code`로 기록(3/3) | 동일 |

지연 수치는 움직였지만 게이트웨이 버전과 클러스터가 함께 바뀌어 차이를
어느 쪽에도 귀속하지 않는다. v1.5.0 열이 정본이다.

| 항목(p50, 20회차 중앙값) | v1.4.1 회차 | v1.5.0 회차 |
|---|---|---|
| 프록시 홉, 매번 새 연결 | +2.3~2.5ms | +3.0ms |
| 프록시 홉, 연결 재사용 | +0.5~1.0ms | +0.8~1.1ms |
| A2A 처리, 연결 재사용 | +0.6~0.8ms | +0.1~0.2ms |
| A2A 처리, 매번 새 연결 | +0.00~+0.05ms | +0.00ms |
| 게이트웨이 p99 대 직접 | 높음(5~10ms) | 높음(3~6ms) |
